...
const warehouseDB = {
  ...
  creditors: [] // {id, name, amount, dueDate, status, payments: []}
};
...
function renderCounts() {
  ...
  document.getElementById("dbCreditorCount").textContent = warehouseDB.creditors.length;

  const totalCredits = (warehouseDB.creditors || [])
...
function addCreditor() {
  ...
  if (!name || amount <= 0 || !dueDate) {
    alert("Name, positive amount, and due date are required.");
    return;
  }

  warehouseDB.creditors = warehouseDB.creditors || [];

  const id = "CR" + (warehouseDB.creditors.length + 1).toString().padStart(3, "0");

  warehouseDB.creditors.push({
    id,
    name,
    amount,
    dueDate,
...
function renderCreditors() {
  const body = document.getElementById("creditorsTableBody");
  body.innerHTML = "";

  warehouseDB.creditors.forEach(c => {
    body.innerHTML += `<tr>
      <td>${c.id}</td>
      <td>${c.name}</td>
      <td>GH₵ ${c.amount.toFixed(2)}</td>
      <td>${c.dueDate}</td>
      <td>${c.status}</td>
    </tr>`;
  });
...
function renderCreditors() {
  ...
  warehouseDB.creditors.forEach(c => {
    body.innerHTML += `<tr>
      ...
      <td>${c.status}</td>
      <td>
        <button class="btn btn-success" onclick="markCreditorPaid('${c.id}')">Mark as Paid</button>
        <button class="btn btn-primary" onclick="partPaymentPrompt('${c.id}')">Part Payment</button>
      </td>
    </tr>`;
  });
}

function markCreditorPaid(id) {
  const creditor = warehouseDB.creditors.find(c => c.id === id);
  if (!creditor) return;

  creditor.payments = creditor.payments || [];
...
function partPaymentPrompt(id) {
  ...
  const payment = parseFloat(prompt(`Enter part payment for ${creditor.name} (Amount owed: GH₵ ${creditor.amount.toFixed(2)})`));
  ...
  if (payment >= creditor.amount) {
    creditor.amount = 0;
    creditor.status = "Paid";
  } else {
    creditor.amount -= payment;
    creditor.status = "Partially Paid";
  }
...
function clearWarehouseData() {
  ...
  warehouseDB.inventory = [];
  warehouseDB.movements = [];
  warehouseDB.creditors = []; // clear creditors too
  saveWarehouseDB();
  renderAll();
}
...
function renderAll() {
  ...
  renderCounts();
  renderMovementsChart();
  renderCreditors(); // ✅ this ensures creditors show after refresh
}
loadWarehouseDB();
